...
class TestBaseModel(unittest.TestCase):
    ...
    def test_initialization_with_args(self):
        ...
        self.assertEqual(base_model.created_at.strftime("%Y-%m-%dT%H:%M:%S.%f"), now)
        self.assertEqual(base_model.updated_at.strftime("%Y-%m-%dT%H:%M:%S.%f"), now)
    ...
    def test_str_representation(self):
        ...
        self.assertEqual(str(base_model), "[BaseModel] (123) {'id': '123'}")

    def test_save_method(self):
        base_model = BaseModel()
        initial_updated_at = base_model.updated_at.strftime("%Y-%m-%dT%H:%M:%S.%f")
        base_model.save()
